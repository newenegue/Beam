  ...
  def index
    ...
    search = params[:search]
  	...
  	if session[:access_token]
        ...
        # search username 
        if search.index("@")
          user_id = session[:client].user_search(search[1..-1])[0].id
        ...
        # search hashtag
          ...
          @instagram_results = session[:client].tag_recent_media(search, {count: 10})
        end
        ...
        # search hashtag
          ...
          @instagram_results = Instagram.tag_recent_media(search, {count: 10})
        end
  			@next_url = @instagram_results.pagination.next_url
  		end
  	end
